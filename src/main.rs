...
async fn main() {
    ...
    loop {
        ...
        while let Some(file) = videos.next().await {
            ...
            // e.g. recording.
            if !try_exclusive(fs::File::open(file.path()).await.unwrap()).unwrap() {
                continue;
            }
            run_encode_when_inactive(file.path(), &settings).await;
        }
...
async fn run_encode_when_inactive(input: std::path::PathBuf, settings: &Settings) {
    let mut output_path = settings.output_folder.join(input.file_name().unwrap());
